import typing, os, re
import reflection_utils, clang_utils, logger_utils, cpp_operator, script_codegen_utils
...
def make_correct_type_for_library(code_structure: reflection_utils.CPPCodeStructure, cpp_type: reflection_utils.CPPType, is_return_type = False) -> str:
...
def make_correct_type_for_api(code_structure: reflection_utils.CPPCodeStructure, cpp_type: reflection_utils.CPPType) -> str:
...
def generate_header_method(f: typing.IO, depth: int, scriptable_class: reflection_utils.ScriptableClass, method: reflection_utils.CPPMethod, code_structure: reflection_utils.CPPCodeStructure):
...
def generate_core_api_fwd_decl(logger: logger_utils.Logger, library_dir: str, code_structure: reflection_utils.CPPCodeStructure):
    with open(library_dir + '/CoreAPIFwd.h', 'w', encoding='utf-8') as f:
        f.write(f'{reflection_utils.sparcle_utils.codegen_cpp_header_comment}')
...
def generate_impl_cpp(logger: logger_utils.Logger, library_dir: str, code_structure: reflection_utils.CPPCodeStructure):
...
def generate_library(logger: logger_utils.Logger, repo_dir: str, library_dir: str, code_structure: reflection_utils.CPPCodeStructure):
