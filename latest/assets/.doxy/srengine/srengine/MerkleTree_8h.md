

# File MerkleTree.h



[**FileList**](files.md) **>** [**Engine**](dir_0b6ee6e7e9547e675db4add2e5de97da.md) **>** [**libs**](dir_864715c2a33e3813fa4e93cb9390f3e1.md) **>** [**Utils**](dir_3df27e53d7cc6cea988e3644dbf87b98.md) **>** [**inc**](dir_27b05de82864f3a1e6ade431259242d5.md) **>** [**Utils**](dir_800651e705081dade93279ddf25689e7.md) **>** [**Types**](dir_5acfcf7a6efc565cfce10e624e0935ad.md) **>** [**MerkleTree.h**](MerkleTree_8h.md)

[Go to the source code of this file](MerkleTree_8h_source.md)



* `#include <Utils/stdInclude.h>`













## Namespaces

| Type | Name |
| ---: | :--- |
| namespace | [**SR\_HTYPES\_NS**](namespaceSR__HTYPES__NS.md) <br>_#define SR\_SHARED\_PTR\_TRACE_  |
| namespace | [**SHA256**](namespaceSR__HTYPES__NS_1_1SHA256.md) <br> |
| namespace | [**Details**](namespaceSR__HTYPES__NS_1_1SHA256_1_1Details.md) <br> |


## Classes

| Type | Name |
| ---: | :--- |
| struct | [**HashT**](structSR__HTYPES__NS_1_1HashT.md) &lt;SIZE&gt;<br>_Template for fixed-size hashes._  |
| class | [**PathT**](classSR__HTYPES__NS_1_1PathT.md) &lt;HASH\_SIZE, HASH\_FUNCTION&gt;<br>_Template for Merkle paths._  |
| struct | [**Element**](structSR__HTYPES__NS_1_1PathT_1_1Element.md) <br>_MerklePath element._  |
| class | [**TreeT**](classSR__HTYPES__NS_1_1TreeT.md) &lt;HASH\_SIZE, HASH\_FUNCTION&gt;<br>_Template for Merkle trees._  |
| struct | [**Statistics**](structSR__HTYPES__NS_1_1TreeT_1_1Statistics.md) <br>_Structure to hold statistical information._  |

















































## Macros

| Type | Name |
| ---: | :--- |
| define  | [**MERKLECPP\_TRACE**](MerkleTree_8h.md#define-merklecpp_trace) (X) <br> |
| define  | [**MERKLECPP\_VERSION\_MAJOR**](MerkleTree_8h.md#define-merklecpp_version_major)  `1`<br> |
| define  | [**MERKLECPP\_VERSION\_MINOR**](MerkleTree_8h.md#define-merklecpp_version_minor)  `0`<br> |
| define  | [**MERKLECPP\_VERSION\_PATCH**](MerkleTree_8h.md#define-merklecpp_version_patch)  `0`<br> |

## Macro Definition Documentation





### define MERKLECPP\_TRACE 

```C++
#define MERKLECPP_TRACE (
    X
) 
```




<hr>



### define MERKLECPP\_VERSION\_MAJOR 

```C++
#define MERKLECPP_VERSION_MAJOR `1`
```




<hr>



### define MERKLECPP\_VERSION\_MINOR 

```C++
#define MERKLECPP_VERSION_MINOR `0`
```




<hr>



### define MERKLECPP\_VERSION\_PATCH 

```C++
#define MERKLECPP_VERSION_PATCH `0`
```




<hr>

------------------------------
The documentation for this class was generated from the following file `Engine/libs/Utils/inc/Utils/Types/MerkleTree.h`

