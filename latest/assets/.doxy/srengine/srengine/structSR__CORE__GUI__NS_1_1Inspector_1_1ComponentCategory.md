

# Struct SR\_CORE\_GUI\_NS::Inspector::ComponentCategory



[**ClassList**](annotated.md) **>** [**ComponentCategory**](structSR__CORE__GUI__NS_1_1Inspector_1_1ComponentCategory.md)


























## Public Attributes

| Type | Name |
| ---: | :--- |
|  std::map&lt; [**SR\_UTILS\_NS::StringAtom**](classSR__UTILS__NS_1_1StringAtom.md), ComponentCategory &gt; | [**categories**](#variable-categories)  <br> |
|  std::vector&lt; [**SR\_UTILS\_NS::StringAtom**](classSR__UTILS__NS_1_1StringAtom.md) &gt; | [**components**](#variable-components)  <br> |












































## Public Attributes Documentation




### variable categories 

```C++
std::map<SR_UTILS_NS::StringAtom, ComponentCategory> SR_CORE_GUI_NS::Inspector::ComponentCategory::categories;
```




<hr>



### variable components 

```C++
std::vector<SR_UTILS_NS::StringAtom> SR_CORE_GUI_NS::Inspector::ComponentCategory::components;
```




<hr>

------------------------------
The documentation for this class was generated from the following file `Engine/inc/Engine/GUI/Inspector.h`

