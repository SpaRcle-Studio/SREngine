

# Struct SR\_CORE\_NS::GUI::FBElement



[**ClassList**](annotated.md) **>** [**SR\_CORE\_NS**](namespaceSR__CORE__NS.md) **>** [**GUI**](namespaceSR__CORE__NS_1_1GUI.md) **>** [**FBElement**](structSR__CORE__NS_1_1GUI_1_1FBElement.md)


























## Public Attributes

| Type | Name |
| ---: | :--- |
|  std::string | [**cutName**](#variable-cutname)  <br> |
|  std::string | [**filename**](#variable-filename)  <br> |
|  EditorIcon | [**iconType**](#variable-icontype)  <br> |
|  bool | [**isDir**](#variable-isdir)  <br> |
|  SR\_GTYPES\_NS::Texture::Ptr | [**pTexture**](#variable-ptexture)  <br> |












































## Public Attributes Documentation




### variable cutName 

```C++
std::string SR_CORE_NS::GUI::FBElement::cutName;
```




<hr>



### variable filename 

```C++
std::string SR_CORE_NS::GUI::FBElement::filename;
```




<hr>



### variable iconType 

```C++
EditorIcon SR_CORE_NS::GUI::FBElement::iconType;
```




<hr>



### variable isDir 

```C++
bool SR_CORE_NS::GUI::FBElement::isDir;
```




<hr>



### variable pTexture 

```C++
SR_GTYPES_NS::Texture::Ptr SR_CORE_NS::GUI::FBElement::pTexture;
```




<hr>

------------------------------
The documentation for this class was generated from the following file `Engine/inc/Engine/GUI/FileBrowser.h`

