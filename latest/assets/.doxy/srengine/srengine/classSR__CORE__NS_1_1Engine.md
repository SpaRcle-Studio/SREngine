...
```C++
SR_NODISCARD const PhysicsScenePtr & SR_CORE_NS::Engine::GetPhysicsScene () const
```
...
```C++
inline SR_NODISCARD const RenderContextPtr & SR_CORE_NS::Engine::GetRenderContext () const
```
...
```C++
SR_NODISCARD const RenderScenePtr & SR_CORE_NS::Engine::GetRenderScene () const
```
