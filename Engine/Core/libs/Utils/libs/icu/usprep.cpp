...
*/
static UHashtable *SHARED_DATA_HASHTABLE_USPREP = NULL;
...
static UVersionInfo dataVersion_USOREP={ 0, 0, 0, 0 };
...
isSPrepAcceptable(void * /* context */,
    ...
    if(
        ...
        uprv_memcpy(dataVersion_USOREP, pInfo->dataVersion, 4);
        return true;
    } else {
        return false;
    }
...
/* hashes an entry  */
static int32_t U_CALLCONV 
hashEntry1(const UHashTok parm) {
...
static UBool U_CALLCONV 
compareEntries1(const UHashTok p1, const UHashTok p2) {
...
usprep_internal_flushCache(UBool noRefCount){
    ...
    if (SHARED_DATA_HASHTABLE_USPREP == NULL) {
        umtx_unlock(&usprepMutex);
        return 0;
    }

    /*creates an enumeration to iterate through every element in the table */
    while ((e = uhash_nextElement(SHARED_DATA_HASHTABLE_USPREP, &pos)) != NULL)
    {
        ...
        if ((noRefCount== false && profile->refCount == 0) || 
             noRefCount== true) {
            deletedNum++;
            uhash_removeElement(SHARED_DATA_HASHTABLE_USPREP, e);
...
static UBool U_CALLCONV usprep_cleanup(void){
    if (SHARED_DATA_HASHTABLE_USPREP != NULL) {
        ...
        if (SHARED_DATA_HASHTABLE_USPREP != NULL && uhash_count(SHARED_DATA_HASHTABLE_USPREP) == 0) {
            uhash_close(SHARED_DATA_HASHTABLE_USPREP);
            SHARED_DATA_HASHTABLE_USPREP = NULL;
        }
    }
    gSharedDataInitOnce.reset();
    return (SHARED_DATA_HASHTABLE_USPREP == NULL);
}
U_CDECL_END
...
createCache1(UErrorCode &status) {
    SHARED_DATA_HASHTABLE_USPREP = uhash_open(hashEntry1, compareEntries1, NULL, &status);
    if (U_FAILURE(status)) {
        SHARED_DATA_HASHTABLE_USPREP = NULL;
    }
...
static void 
initCache1(UErrorCode *status) {
    umtx_initOnce(gSharedDataInitOnce, &createCache1, *status);
}
...
loadData(UStringPrepProfile* profile, 
    ...
    sprepUniVer = (dataVersion_USOREP[0] << 24) + (dataVersion_USOREP[1] << 16) +
                  (dataVersion_USOREP[2] << 8 ) + (dataVersion_USOREP[3]);
...
usprep_getProfile(const char* path, 
    ...
    umtx_lock(&usprepMutex);
    profile = (UStringPrepProfile*) (uhash_get(SHARED_DATA_HASHTABLE_USPREP,&stackKey));
    if(profile != NULL) {
        profile->refCount++;
    }
    umtx_unlock(&usprepMutex);
    ...
    if(profile == NULL) {
        ...
        if(profile != NULL) {
            ...
            profile->refCount = 1;
            uhash_put(SHARED_DATA_HASHTABLE_USPREP, key.orphan(), profile, status);
        }
        umtx_unlock(&usprepMutex);
    }
