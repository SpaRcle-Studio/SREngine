...
UnicodeString& ICU_Utility::appendNumber(UnicodeString& result, int32_t n,
    ...
    while (--minDigits > 0) {
        result.append(DIGITS_UTIL[0]);
    }
    while (r > 0) {
        int32_t digit = n / r;
        result.append(DIGITS_UTIL[digit]);
        n -= digit * r;
        r /= radix;
    }
    return result;
...
UnicodeString &ICU_Utility::escape(UnicodeString& result, UChar32 c) {
    result.append(BACKSLASH_UTIL);
    if (c & ~0xFFFF) {
        result.append(UPPER_U);
        result.append(DIGITS_UTIL[0xF&(c>>28)]);
        result.append(DIGITS_UTIL[0xF&(c>>24)]);
        result.append(DIGITS_UTIL[0xF&(c>>20)]);
        result.append(DIGITS_UTIL[0xF&(c>>16)]);
    } else {
        result.append(LOWER_U);
    }
    result.append(DIGITS_UTIL[0xF&(c>>12)]);
    result.append(DIGITS_UTIL[0xF&(c>>8)]);
    result.append(DIGITS_UTIL[0xF&(c>>4)]);
    result.append(DIGITS_UTIL[0xF&c]);
    return result;
}
...
void ICU_Utility::appendToRule(UnicodeString& rule,
    ...
    if (isLiteral ||
        ...
        if (quoteBuf.length() > 0) {
            ...
            while (quoteBuf.length() >= 2 &&
                ...
                rule.append(BACKSLASH_UTIL).append(APOSTROPHE);
                quoteBuf.remove(0, 2);
            }
    ...
    // Escape ' and '\' and don't begin a quote just for them
             ...
             (c == APOSTROPHE || c == BACKSLASH_UTIL)) {
        rule.append(BACKSLASH_UTIL);
        rule.append(c);
    }
