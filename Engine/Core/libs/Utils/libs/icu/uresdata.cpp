...
res_read(ResourceData *pResData,
    ...
    if(!isAcceptable1(formatVersion, NULL, NULL, pInfo)) {
        *errorCode=U_INVALID_FORMAT_ERROR;
        return;
    }
...
res_load(ResourceData *pResData,
    ...
    pResData->data=udata_openChoice(path, "res", name, isAcceptable1, formatVersion, errorCode);
    if(U_FAILURE(*errorCode)) {
        return;
    }
...
enum {
    STACK_ROW_CAPACITY_URESDATA=200
};
...
ures_swap(const UDataSwapper *ds,
    ...
    Row rows[STACK_ROW_CAPACITY_URESDATA];
    int32_t resort[STACK_ROW_CAPACITY_URESDATA];
    TempTable tempTable;
    ...
    if(length>=0) {
        ...
        uint32_t stackResFlags[STACK_ROW_CAPACITY_URESDATA];
        ...
        if(tempTable.majorFormatVersion>1 || maxTableLength<=STACK_ROW_CAPACITY_URESDATA) {
            tempTable.rows=rows;
