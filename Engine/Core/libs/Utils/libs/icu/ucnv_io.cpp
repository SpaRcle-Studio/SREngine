...
static const char DATA_TYPE_UCNV_IO[] = "icu";
...
static void U_CALLCONV initAliasData(UErrorCode &errCode) {
    ...
    data = udata_openChoice(NULL, DATA_TYPE_UCNV_IO, DATA_NAME, isAcceptable, NULL, &errCode);
    ...
    while ((c1 = *name++) != 0) {
        ...
        case UCNV_ZERO:
            if (!afterDigit) {
                ...
                if (nextType == UCNV_ZERO || nextType == NONZERO) {
