...
TODO: This cache should probably be removed when the deprecated code is
      completely removed.
*/
static UHashtable *cache1 = NULL;
static icu::UInitOnce gCacheInitOnce1 {};
...
static int32_t ures_flushCache()
{
    ...
    */
    ...
    do {
        ...
        pos = UHASH_FIRST;
        while ((e = uhash_nextElement(cache1, &pos)) != NULL)
        {
...
U_CAPI UBool U_EXPORT2 ures_dumpCacheContents(void) {
  ...
  UResourceDataEntry *resB;
    ...
    fprintf(stderr,"%s:%d: RB Cache still contains %d items.\n", __FILE__, __LINE__, uhash_count(cache1));
...
static UBool U_CALLCONV ures_cleanup(void)
{
    ...
    gCacheInitOnce1.reset();
    return true;
}

/** INTERNAL: Initializes the cache for resources */
static void U_CALLCONV createCache(UErrorCode &status) {
    U_ASSERT(cache1 == NULL);
    cache1 = uhash_open(hashEntry, compareEntries, NULL, &status);
    ucln_common_registerCleanup(UCLN_COMMON_URES, ures_cleanup);
}
     
static void initCache(UErrorCode *status) {
    umtx_initOnce(gCacheInitOnce1, &createCache, *status);
}
...
static UResourceDataEntry *init_entry(const char *localeID, const char *path, UErrorCode *status) {
    ...
    r = (UResourceDataEntry *)uhash_get(cache1, &find);
    if(r == NULL) {
...

        {
            ...
            if((oldR = (UResourceDataEntry *)uhash_get(cache1, r)) == NULL) { /* if the data is not cached */
                /* just insert it in the cache */
                UErrorCode cacheStatus = U_ZERO_ERROR;
                uhash_put(cache1, (void *)r, r, &cacheStatus);
