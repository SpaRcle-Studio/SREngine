...
UChar32 getRangeUCPTRIE(const void *t, UChar32 start,
...
ucptrie_internalGetRange(UCPTrieGetRange *getRange,
    ...
    if (option == UCPMAP_RANGE_NORMAL) {
        return getRangeUCPTRIE(trie, start, filter, context, pValue);
    }
    uint32_t value;
    if (pValue == nullptr) {
    ...
    UChar32 end = getRangeUCPTRIE(trie, start, filter, context, pValue);
    ...
    uint32_t value2;
    UChar32 end2 = getRangeUCPTRIE(trie, surrEnd + 1, filter, context, &value2);
...
ucptrie_getRange(const UCPTrie *trie, UChar32 start,
                 UCPMapRangeOption option, uint32_t surrogateValue,
                 UCPMapValueFilter *filter, const void *context, uint32_t *pValue) {
    return ucptrie_internalGetRange(getRangeUCPTRIE, trie, start,
