...
static void
writeBlock1(uint32_t *block, uint32_t value) {
    ...
    while(start<limit) {
        ...
        if(setRepeatBlock) {
            if(repeatBlock>=0) {
                ...
                writeBlock1(newTrie->data+repeatBlock, value);
            }
        }
...
static inline UBool
equal_uint321(const uint32_t *s, const uint32_t *t, int32_t length) {
    while(length>0 && *s==*t) {
        ++s;
        ++t;
        --length;
...
findSameDataBlock(const uint32_t *data, int32_t dataLength, int32_t otherBlock, int32_t blockLength) {
    ...
    for(block=0; block<=dataLength; block+=UTRIE2_DATA_GRANULARITY) {
        if(equal_uint321(data+block, data+otherBlock, blockLength)) {
            return block;
        }
    }
    return -1;
...
compactData(UNewTrie2 *trie) {
    ...
    for(start=newStart; start<trie->dataLength;) {
        ...
        for(overlap=blockLength-UTRIE2_DATA_GRANULARITY;
            overlap>0 && !equal_uint321(trie->data+(newStart-overlap), trie->data+start, overlap);
