...
void Guizmo::DrawManipulation(SR_GRAPH_NS::Camera* camera) {
    ...
    if (ImGuizmo::IsUsing()) {
        Math::FVector3 translation, rotation, scale;
...
void Guizmo::SetRect(SR_GRAPH_NS::Camera* camera) {
    ...
    const auto winSize = Math::FVector2(window->Size.x, window->Size.y);
...
glm::mat4 Guizmo::GetMatrix() {
    ...
    const Math::FVector3 translation = m_transform->GetTranslation().InverseAxis(0);
    const Math::FVector3 rotation = m_transform->GetRotation().InverseAxis(1).InverseAxis(2);
    const Math::FVector3 scale = m_transform->GetScale();

    matrix = glm::translate(matrix, translation.ToGLM());
    matrix *= mat4_cast(Math::Quaternion::FromEuler(rotation).ToGLM());
...
void Guizmo::OnKeyPress(const KeyPressEvent &event) {
    switch (event.GetKeyCode()) {
        case Helper::KeyCode::F: {
            m_transform->RotateAroundParent(Math::FVector3(0, 1, 0));
            break;
        }
        default:
            break;
