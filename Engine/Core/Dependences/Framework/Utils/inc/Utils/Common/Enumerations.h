...
#define SR_ENUM_CODEGEN(enumName, ...)                                                           \
    namespace SR_CODEGEN_ENUM {                                                                  \
        ...
        static void procedure##enumName##_enum_parse() {                                         \
            auto tokens = SR_UTILS_NS::StringUtils::Split(#__VA_ARGS__, ",");                    \
            int64_t last = 0;                                                                    \
            auto& _mapKey = codegen_get_##enumName##_mapKey();                                   \
            auto& _mapValue = codegen_get_##enumName##_mapValue();                               \
            for (std::string& token : tokens) {                                                  \
                token = SR_UTILS_NS::StringUtils::RemoveCharsFromString(token, " ");             \
                if (token.find('=') != std::string::npos) {                                      \
                    auto[name, value] = SR_UTILS_NS::StringUtils::SplitTwo(token, "=");          \
                    last = std::stoi(value);                                                     \
                    token = name;                                                                \
                }                                                                                \
