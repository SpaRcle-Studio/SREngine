...
namespace SR_CORE_NS::Tests {
    ...
    public:
        static bool Run() {
            ...
            // auto&& pPage =
            // SR_UTILS_NS::Web::HTMLParser::Instance().Parse(path); const
            // std::string text =
            // SR_UTILS_NS::Web::HTMLParser::Instance().DebugPageTostring(pPage);
            // SR_DEBUG_LOG("{}", text);
            return true;
        }
    };
    ...
    public:
        static bool Run() {
            ...
            // auto&& pCSS =
            // SR_UTILS_NS::Web::CSSParser::Instance().Parse(path); const
            // std::string text = pCSS->ToString(); SR_DEBUG_LOG("{}", text);
            return true;
        }
    };
} // namespace SR_CORE_NS::Tests
