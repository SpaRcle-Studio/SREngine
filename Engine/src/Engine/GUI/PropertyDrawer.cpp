...
namespace SR_CORE_GUI_NS {
    ...
    PropertyDrawerFeedback BoolPropertyDrawer::Draw(const PropertyDrawerContext& context) {
        ...
        if (auto&& bValue = value.TryCast<bool>()) {
            ...
            SR_GRAPH_GUI_NS::Immediate::TextColored(
                SR_MATH_NS::FColor(1.f, 0.f, 0.f, 1.f), "Failed to map bool value!"
            );
        }
    ...
    PropertyDrawerFeedback NumericPropertyDrawer::Draw(const PropertyDrawerContext& context) {
        ...
        if (!context.pValue &&
            SR_GRAPH_GUI_NS::Immediate::Button(context.GetEditorParams().GetDisplayName().c_str(), buttonSize)) {
            if (context.onBeforeChangeCallback) {
                context.onBeforeChangeCallback(false);
            }
            feedback.isChanged = true;
        ...
        const auto dataType =
            SR_GRAPH_GUI_NS::Immediate::GetDataType(value.SizeOf(), value.IsSigned(), value.IsIntegral());
        ...
        if (void* pValue = value.Data()) {
            SR_GRAPH_GUI_NS::Immediate::ImmediateDataTypeUnion dataTypeUnion =
                SR_GRAPH_GUI_NS::Immediate::ReadDataType(pValue, dataType);
            ...
            SR_GRAPH_GUI_NS::Immediate::TextColored(
                SR_MATH_NS::FColor(1.f, 0.f, 0.f, 1.f), "Failed to map numeric value!"
            );
        }
    ...
    PropertyDrawerFeedback MathVectorPropertyDrawer::Draw(const PropertyDrawerContext& context) {
        ...
        for (uint8_t i = 0; i < dimension; ++i) {
            ...
            if (partSize == 1) {
                ...
                SR_GRAPH_GUI_NS::Immediate::SameLine();
                SR_GRAPH_GUI_NS::Immediate::Dummy(
                    SR_MATH_NS::FVector2(partItemWidth - SR_GRAPH_GUI_NS::Immediate::GetFrameHeight(), 0.0f)
                );
                SR_GRAPH_GUI_NS::Immediate::PushItemWidth(partItemWidth);
    ...
    PropertyDrawerFeedback MathSizePropertyDrawer::Draw(const PropertyDrawerContext& context) {
        ...
        if (!context.pValue) {
            ...
            if (SR_GRAPH_GUI_NS::Immediate::Button(
                    context.GetProperty().GetEditorParams().GetDisplayName().c_str(), mainButtonSize
                )) {
                value = context.GetProperty().GetResetValue() ? context.GetProperty().GetResetValue()
                                                              : context.GetProperty().GetDefaultValue();
                value = value.DetachIfConst();
        ...
        SR_GRAPH_GUI_NS::Immediate::BeginGroup();
        {
            ...
            for (uint8_t i = 0; i < dimension; ++i) {
                ...
                auto&& pFSize =
                    reinterpret_cast<SR_MATH_NS::FSize*>(&((char*)(copy.Data()))[i * sizeof(SR_MATH_NS::FSize)]);
                auto&& pISize =
                    reinterpret_cast<SR_MATH_NS::ISize*>(&((char*)(copy.Data()))[i * sizeof(SR_MATH_NS::ISize)]);
                auto&& pUSize =
                    reinterpret_cast<SR_MATH_NS::USize*>(&((char*)(copy.Data()))[i * sizeof(SR_MATH_NS::USize)]);
                ...
                if (SR_GRAPH_GUI_NS::Immediate::Button(labels[i], buttonSize)) {
                    ...
                    if (value.SizeOf() == context.GetProperty().GetResetValue().SizeOf()) {
                        ...
                        case SR_GRAPH_GUI_NS::Immediate::ImmediateDataType::Float:
                            pFSize->v =
                                reinterpret_cast<SR_MATH_NS::FSize*>(&((char*)(pMapped))[i * sizeof(SR_MATH_NS::FSize)])
                                    ->v;
                            break;
                        case SR_GRAPH_GUI_NS::Immediate::ImmediateDataType::Int32:
                            pISize->v =
                                reinterpret_cast<SR_MATH_NS::ISize*>(&((char*)(pMapped))[i * sizeof(SR_MATH_NS::ISize)])
                                    ->v;
                            break;
                        case SR_GRAPH_GUI_NS::Immediate::ImmediateDataType::UInt32:
                            pUSize->v =
                                reinterpret_cast<SR_MATH_NS::USize*>(&((char*)(pMapped))[i * sizeof(SR_MATH_NS::USize)])
                                    ->v;
                            break;
                        default:
                ...
                default:
                    SR_GRAPH_GUI_NS::Immediate::TextColored(
                        SR_MATH_NS::FColor(1.f, 0.f, 0.f, 1.f), "Unknown part type!"
                    );
                    break;
                }
            ...
            for (uint8_t i = 0; i < dimension; ++i) {
                ...
                if (pSize && SR_GRAPH_GUI_NS::Immediate::BeginCombo(
                                 "", SR_UTILS_NS::EnumReflector::ToStringAtom(*pSize).c_str()
                             )) {
