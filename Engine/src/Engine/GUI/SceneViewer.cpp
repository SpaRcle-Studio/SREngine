...
namespace SR_CORE_GUI_NS {
    ...
    SceneViewer::~SceneViewer() { Enable(false); }
    ...
    void SceneViewer::Draw() {
        ...
        if (m_enabled && !m_platform && m_isPrefab) {
            ...
            if (!m_platform) {
                m_platform = m_scene->InstanceFromFile("Engine/Models/plane_extended.obj")
                                 .DynamicCast<SR_UTILS_NS::GameObject>();
            }
            if (m_platform) {
                ...
                if (!m_platform->GetChildren().empty()) {
                    if (auto&& pMesh = m_platform->GetChildren()[0]->GetComponent<SR_GTYPES_NS::Mesh3D>()) {
                        ...
                        pMaterial->SetTexture(
                            "diffuse"_atom,
                            GetRenderScene()->GetRenderStrategy()->GetRenderContext()->GetDefaultTexture()
                        );
        ...
        if (SR_GRAPH_GUI_NS::Immediate::BeginChild("ViewerTexture")) {
            m_windowSize = SR_MATH_NS::Vector2(
                static_cast<int32_t>(SR_GRAPH_GUI_NS::Immediate::GetWindowSize().x),
                static_cast<int32_t>(SR_GRAPH_GUI_NS::Immediate::GetWindowSize().y)
            );
    ...
    void SceneViewer::Enable(bool value) {
        ...
        /// если сцена сломана, или это "пустышка", то не создаем камеру, т.к.
        /// рендерить нет смыла
        if (!pLogic || !m_enabled) {
    ...
    void
    SceneViewer::DrawTexture(SR_MATH_NS::IVector2 winSize, SR_MATH_NS::IVector2 texSize, uint32_t id, bool centralize) {
    ...
    void SceneViewer::SetCameraEnabled(bool enabled) {
        ...
        if (enabled) {
            ...
            if (m_isPrefab) {
                pCameraComponent->SetRenderTechnique(
                    SR_CORE_NS::EditorSettings::Instance().GetPrefabEditorRenderTechnique()
                );
            } else {
    ...
    void SceneViewer::OnClose() { Enable(false); }

    void SceneViewer::OnOpen() { Enable(true); }
    ...
    void SceneViewer::BackupCameraSettings() {
        ...
        if (!xmlDocument.Save(SR_UTILS_NS::ResourceManager::Instance().GetCachePath().Concat(CAMERA_XML))) {
            SR_ERROR("SceneViewer::BackupCameraSettings() : failed to save "
                     "camera settings!");
        }
    }
    ...
    bool SceneViewer::UpdateViewSize() {
        auto&& pCamera = m_camera->GetComponent<EditorCamera>();
        if (!pCamera) {
            SR_WARN("SceneViewer::UpdateViewSize() : editor camera component "
                    "not found!");
            return false;
        }
