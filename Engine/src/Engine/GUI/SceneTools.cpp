...
namespace SR_CORE_GUI_NS {
    void SceneTools::Init() {
        ...
        AddElement("L").SetIsActive([this]() { return m_gizmoMode == SR_GRAPH_UI_NS::GizmoMode::Local; }
        ).SetOnClick([this](bool isActive) {
            if (isActive) {
                SetGizmoMode(SR_GRAPH_UI_NS::GizmoMode::Global);
            } else {
                SetGizmoMode(SR_GRAPH_UI_NS::GizmoMode::Local);
            }
        });
        ...
        AddElement("T")
            ...
            .SetOnClick([this](bool isActive) {
                if (isActive) {
                    SetGizmoOperation(SR_GRAPH_UI_NS::GizmoOperation::None);
                } else {
                    SetGizmoOperation(SR_GRAPH_UI_NS::GizmoOperation::Translate);
                }
            });

        AddElement("R").SetIsActive([this]() { return m_gizmoOperationFlag & SR_GRAPH_UI_NS::GizmoOperation::Rotate; }
        ).SetOnClick([this](bool isActive) {
            if (isActive) {
                SetGizmoOperation(SR_GRAPH_UI_NS::GizmoOperation::None);
            } else {
                SetGizmoOperation(SR_GRAPH_UI_NS::GizmoOperation::Rotate);
            }
        });

        AddElement("S").SetIsActive([this]() { return m_gizmoOperationFlag & SR_GRAPH_UI_NS::GizmoOperation::Scale; }
        ).SetOnClick([this](bool isActive) {
            if (isActive) {
                SetGizmoOperation(SR_GRAPH_UI_NS::GizmoOperation::None);
            } else {
                SetGizmoOperation(SR_GRAPH_UI_NS::GizmoOperation::Scale);
            }
        });
        ...
        AddElement("Connect PVD")
            ...
            .SetOnClick([](bool isActive) {
                auto&& pLibrary =
                    SR_PHYSICS_NS::PhysicsLibrary::Instance().GetActiveLibrary(SR_UTILS_NS::Measurement::Space3D);
                if (pLibrary) {
                    pLibrary->ConnectPVD();
                }
            })
        ...
        AddElement()
            .SetCustomDraw([this](auto&& pElement) {
                ...
                if (SR_GRAPH_GUI_NS::Immediate::BeginCombo(
                        "View Mode", SR_UTILS_NS::EnumReflector::ToStringAtom(m_viewMode).c_str()
                    )) {
        ...
        AddElement("Re-Draw")
            .SetIsActive([]() { return false; })
            .SetOnClick([&](bool isActive) { GetRenderScene()->GetPipeline()->SetDirty(true); })
            .SetWidth(80.f)
            .SetItemSpacing(SR_MATH_NS::FVector2(10.f, 0.f));
    ...
    void SceneTools::OnKeyDown(const SR_UTILS_NS::KeyboardInputData* pData) {
        ...
        case SR_UTILS_NS::KeyCode::_1:
            SetGizmoOperation(SR_GRAPH_UI_NS::GizmoOperation::None);
            break;
        case SR_UTILS_NS::KeyCode::_2:
            SetGizmoOperation(SR_GRAPH_UI_NS::GizmoOperation::Translate);
            break;
        case SR_UTILS_NS::KeyCode::_3:
            SetGizmoOperation(SR_GRAPH_UI_NS::GizmoOperation::Rotate);
            break;
        case SR_UTILS_NS::KeyCode::_4:
            SetGizmoOperation(SR_GRAPH_UI_NS::GizmoOperation::Scale);
            break;
        default:
            break;
