...
namespace SR_CORE_GUI_NS {
    PropertyDrawerFeedback StringPropertyDrawer::Draw(const PropertyDrawerContext& context) {
        ...
        const bool isTextBox =
            context.pProperty && context.pProperty->GetEditorParams().GetCustomArg("text-box") == "enabled";
        ...
        if (value.IsString()) {
            if (auto&& pString = value.TryCast<std::string>()) {
                ...
                SR_GRAPH_GUI_NS::Immediate::TextColored(
                    SR_MATH_NS::FColor(1.f, 0.f, 0.f, 1.f), "Failed to map string value!"
                );
            }
            ...
            if (auto&& pStringView = value.TryCast<std::string_view>()) {
                ...
                if (pStringView->empty()) {
                    ...
                    SR_GRAPH_GUI_NS::Immediate::InputText(
                        "##Input", emptyString.data(), emptyString.size(),
                        SR_GRAPH_GUI_NS::Immediate::InputTextFlags::ReadOnly
                    );
                } else {
                    SR_GRAPH_GUI_NS::Immediate::InputText(
                        "##Input", const_cast<char*>(pStringView->data()), pStringView->size(),
                        SR_GRAPH_GUI_NS::Immediate::InputTextFlags::ReadOnly
                    );
                }
            } else {
                SR_GRAPH_GUI_NS::Immediate::TextColored(
                    SR_MATH_NS::FColor(1.f, 0.f, 0.f, 1.f), "Failed to map string view value!"
                );
            }
            ...
            if (auto&& pUnicodeString = value.TryCast<SR_HTYPES_NS::UnicodeString>()) {
                ...
                if (isTextBox) {
                    SR_GRAPH_GUI_NS::Immediate::InputTextMultiline(
                        "##Input", &text, SR_MATH_NS::FVector2(context.fieldWidth, 100)
                    );
                } else {
                ...
                if (SR_GRAPH_GUI_NS::Immediate::IsItemDeactivatedAfterEdit()) {
                    SetMappedValue(
                        context, feedback, pUnicodeString,
                        SR_HTYPES_NS::UnicodeString(SR_UTILS_NS::Localization::UtfToUtf<char32_t, char>(text))
                    );
                }
            } else {
                SR_GRAPH_GUI_NS::Immediate::TextColored(
