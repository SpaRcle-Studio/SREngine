...
namespace SR_CORE_GUI_NS {
    PropertyDrawerFeedback PathPropertyDrawer::Draw(const PropertyDrawerContext& context) {
        ...
        if (!context.pValue || !context.GetPropertyDisplayName().empty()) {
            ...
            const SR_MATH_NS::FVector2 buttonSize =
                isPickingEnabled ? SR_MATH_NS::FVector2(context.fieldTitleWidth * 0.75f, context.fieldHeight)
            ...
            if (isPickingEnabled) {
                ...
                if (SR_GRAPH_GUI_NS::Immediate::Button(
                        "Pick", SR_MATH_NS::FVector2(context.fieldTitleWidth * 0.25f, context.fieldHeight)
                    )) {
                    ...
                    if (!filterName.empty() && !filterValue.empty()) {
                        ...
                        auto&& path =
                            SR_UTILS_NS::FileDialog::Instance().OpenDialog(resourcesPath, {{filterName, filterValue}});
        ...
        if (auto&& pPath = value.TryCast<SR_UTILS_NS::Path>()) {
            ...
            if (SR_GRAPH_GUI_NS::Immediate::InputText(
                    "##Input", &path, SR_GRAPH_GUI_NS::Immediate::InputTextFlags::EnterReturnsTrue
                )) {
                if (context.onBeforeChangeCallback) {
                    context.onBeforeChangeCallback(false);
                }
                feedback.isChanged = true;
                /// If you copy path in Windows in Explorer, then it will be in
                /// quotes, so we need to remove them
                path = SR_UTILS_NS::StringUtils::RemoveCharsFromString(path, "\"");
                *pPath = path;
            }
        } else {
            SR_GRAPH_GUI_NS::Immediate::TextColored(
                SR_MATH_NS::FColor(1.f, 0.f, 0.f, 1.f), "Failed to map path value!"
            );
        }
