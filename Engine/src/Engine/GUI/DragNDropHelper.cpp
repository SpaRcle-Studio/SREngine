...
namespace SR_CORE_GUI_NS {
    bool
    DragDropTargetEntityRef(EditorGUI* pContext, SR_UTILS_NS::EntityRef& entityRef, const char* id, float_t width) {
        std::string preview;
        ...
        SR_GRAPH_GUI_NS::Immediate::PushStyleColor(
            SR_GRAPH_GUI_NS::Immediate::StyleColor::Border, SR_MATH_NS::FColor(247, 84, 225, 255) / 255.f
        );

        if (SR_GRAPH_GUI_NS::Immediate::BeginChild(
                SR_FORMAT_C("{}-{}", id, (void*)&entityRef), SR_MATH_NS::FVector2(width, 50), true
            )) {
            ...
            if (SR_GRAPH_GUI_NS::Immediate::BeginChild(
                    SR_FORMAT_C("{}-{}-1", id, (void*)&entityRef), SR_MATH_NS::FVector2(200, 20), true
                )) {
                ...
                if (SR_GRAPH_GUI_NS::Immediate::BeginDragDropTarget()) {
                    if (auto&& payload = SR_GRAPH_GUI_NS::Immediate::AcceptDragDropPayload("Hierarchy##Payload")) {
                        if (auto&& pData = SR_GRAPH_GUI_NS::Immediate::GetDataFromDragDropPayload(payload)) {
                            std::list<SR_UTILS_NS::SceneObject::Ptr> gameObjects =
                                *(std::list<SR_UTILS_NS::SceneObject::Ptr>*)(pData);
                    ...
                    if (auto&& payload =
                            SR_GRAPH_GUI_NS::Immediate::AcceptDragDropPayload("InspectorComponent##Payload")) {
                        if (auto&& pData = SR_GRAPH_GUI_NS::Immediate::GetDataFromDragDropPayload(payload)) {
                            std::list<SR_UTILS_NS::Component::Ptr> components =
                                *(std::list<SR_UTILS_NS::Component::Ptr>*)(pData);
                            if (!components.empty()) {
            ...
            if (SR_GRAPH_GUI_NS::Immediate::Checkbox(
                    SR_FORMAT_C("Relative##{}-{}", id, (void*)&entityRef), &relative
                )) {
