...
namespace Framework::Graphics {
    ...
    namespace ObjLoader {
        ...
        std::vector<Types::Mesh*> Load(const std::string& path, bool withIndices, Types::MeshType type);

        template<typename T> std::vector<Types::Mesh*> Load(const std::string& path, bool withIndices) {
            auto meshes = std::vector<Types::Mesh*>();

            auto shapes = LoadSourceWithIndices<T::VertexType>(path);
            ...
            for (const auto &[name, indices, vertices] : shapes) {
                ...
                if (withIndices) {
                    ...
                    auto rawVertices = std::vector<T::VertexType>();
        ...
        template<typename T> static std::vector<Shape<T>> LoadSourceWithIndices(const std::string& path) {
            ...
            for (auto& shape : shapes) {
                ...
                source.emplace_back(Shape { shape.name, indices, vertices });
            }
            return source;
        }
    };
