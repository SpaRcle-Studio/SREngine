...
namespace SR_AUDIO_NS {
    SoundDevice::SoundDevice(AudioLibrary library, const std::string& name) : m_library(library), m_name(name) {}
    ...
    SoundDevice* SoundDevice::Allocate(AudioLibrary audioLibrary, const std::string& name) {
        if (name.empty()) {
            ...
            SR_LOG(
                "SoundDevice::Allocate() : allocating a sound device with name "
                "\"" +
                name + "\"..."
            );
        }
    ...
    std::string SoundDevice::GetName() const { return m_name; }

    AudioLibrary SoundDevice::GetLibrary() const { return m_library; }
} // namespace SR_AUDIO_NS