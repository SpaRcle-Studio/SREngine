...
namespace SR_AUDIO_NS::Tools {
    ...
    static int16_t ConvertNibble_MSADPCM(sADPCMDecoderStatus_MS* Status, int Nibble) {
        ...
        int Predictor = (Status->m_Sample1 * Status->m_Coeff1 + Status->m_Sample2 * Status->m_Coeff2) / 256 +
                        SignedNibble * Status->m_iDelta;
...
#define GetWord(w)                                                                                                     \
    ...
    int16_t* Decode_MSADPCM_Block(const uint8_t* Src, int16_t* Dst, size_t NumBytes, bool IsStereo) {
        ...
        if (IsStereo)
            GetWord(StatusRight.m_Sample1);
        GetWord(StatusLeft.m_Sample2);
        if (IsStereo)
            GetWord(StatusRight.m_Sample2);

        // output initial samples
        *Dst++ = StatusLeft.m_Sample1;
        if (IsStereo)
            *Dst++ = StatusRight.m_Sample1;
        *Dst++ = StatusLeft.m_Sample2;
        if (IsStereo)
            *Dst++ = StatusRight.m_Sample2;
    ...
    static int adpcm_index_table[16] = {-1, -1, -1, -1, 2, 4, 6, 8, -1, -1, -1, -1, 2, 4, 6, 8};

    // http://wiki.multimedia.cx/index.php?title=IMA_ADPCM
    int adpcm_step_table[89] = {7,     8,     9,     10,    11,    12,    13,    14,    16,    17,    19,    21,
    ...
    uint16_t ConvertNibble_IMAADPCM(sADPCMDecoderStatus_IMA* Status, int Nibble) {
        ...
        if (Nibble & 0x04)
            Diff += adpcm_step_table[Status->m_StepIndex];
        if (Nibble & 0x02)
            Diff += adpcm_step_table[Status->m_StepIndex] >> 1;
        if (Nibble & 0x01)
            Diff += adpcm_step_table[Status->m_StepIndex] >> 2;
        if (Nibble & 0x08)
            Status->m_Predictor -= Diff;
        else
            Status->m_Predictor += Diff;
    ...
    void
    ConvertClamp_IMAADPCMToInt16(const uint8_t* Src, int16_t* Dst, size_t NumBytes, int BlockAlign, bool IsStereo) {
        const size_t NumBlocks = NumBytes / BlockAlign;
    ...
    int16_t ALaw_Decode(int8_t N) {
        ...
        int16_t Decoded =
            (Position != 4) ? ((1 << Position) | ((N & 0x0F) << (Position - 4)) | (1 << (Position - 5))) : (N << 1) | 1;
...
namespace SR_AUDIO_NS {
    WAVDataProvider::WAVDataProvider(const RawSoundDataPtr& data)
        ...
        if (data && data->size() > sizeof(sWAVHeader)) {
            ...
            /// if (IsRIFF && IsWAVE && ( !IsPCM || IsADPCM_MS || IsADPCM_IMA ||
            /// IsALaw || IsMuLaw ) && IsVerbose())
            /// {
            ///     printf( "Channels       : %i\n", Header->Channels );
            ...
            const bool IsSupportedCodec =
                IsPCM || IsFloat || IsExtFormat || IsADPCM_MS || IsADPCM_IMA || IsALaw || IsMuLaw || IsOLIGSM;
            ...
            if (IsRIFF && IsWAVE && IsSupportedCodec) {
                ...
                if (IsExtFormat) {
                    ...
                    const uint16_t SubFormatTag =
                        *reinterpret_cast<const uint16_t*>(data.get()->data() + HeaderSize + 6);

                    if (SubFormatTag == FORMAT_PCM)
                        IsFloat = false;
                    if (SubFormatTag == FORMAT_FLOAT)
                        IsFloat = true;
                    if (SubFormatTag == FORMAT_ALAW)
                        IsALaw = true;
                    if (SubFormatTag == FORMAT_MULAW)
                        IsMuLaw = true;
                }
                ...
                for (;;) {
                    const sWAVChunkHeader* LocalChunkHeader =
                        reinterpret_cast<const sWAVChunkHeader*>(data.get()->data() + Offset);
                    ...
                    if (memcmp(LocalChunkHeader->ID, "data", 4) == 0) {
                        ...
                        SR_ERROR(
                            "WAVDataProvider::WAVDataProvider() : unknown "
                            "chunk ID: {}{}{}{}\n\tFormated chunk id: {}",
                            LocalChunkHeader->ID[0], LocalChunkHeader->ID[1], LocalChunkHeader->ID[2],
                            LocalChunkHeader->ID[3], chunkID
                        );
                ...
                if (m_dataSize == 0) {
                    SR_ERROR("WAVDataProvider::WAVDataProvider() : no data "
                             "chunk found in WAV");
                    return;
                }
                ...
                if (IsALaw) {
                    ...
                    const uint8_t* Src =
                        reinterpret_cast<const uint8_t*>(m_data.get()->data() + Offset + ChunkHeaderSize);
                    ...
                    Tools::ConvertClamp_MSADPCMToInt16(
                        Src, Dst, m_dataSize, Header->nBlockAlign, Header->Channels == 2
                    );
                    m_data = std::make_shared<std::vector<uint8_t>>(NewData);
                    m_format.m_bitsPerSample = 16;
                    ...
                    const uint8_t* Src =
                        reinterpret_cast<const uint8_t*>(m_data.get()->data() + Offset + ChunkHeaderSize);
                    Tools::ConvertClamp_IMAADPCMToInt16(
                        Src, Dst, m_dataSize, Header->nBlockAlign, Header->Channels == 2
                    );
                    m_data = std::make_shared<std::vector<uint8_t>>(NewData);
                    m_format.m_bitsPerSample = 16;
                    ...
                    if (Header->nBitsperSample == 32) {
                        const float* Src =
                            reinterpret_cast<const float*>(m_data.get()->data() + Offset + ChunkHeaderSize);
                        Tools::ConvertClamp_IEEEToInt16<float>(Src, Dst, m_dataSize / 4);
                        m_dataSize = m_dataSize / 2;
                    } else if (Header->nBitsperSample == 64) {
                        const double* Src =
                            reinterpret_cast<const double*>(m_data.get()->data() + Offset + ChunkHeaderSize);
                        Tools::ConvertClamp_IEEEToInt16<double>(Src, Dst, m_dataSize / 8);
                        m_dataSize = m_dataSize / 4;
                    } else {
                        SR_ERROR("WAVDataProvider::WAVDataProvider() : unknown "
                                 "float format in WAV");
                        m_dataSize = 0;
                    }
                    ...
                    const uint8_t* Src =
                        reinterpret_cast<const uint8_t*>(m_data.get()->data() + Offset + ChunkHeaderSize - 4);
                    ...
                    const int32_t* Src =
                        reinterpret_cast<const int32_t*>(m_data.get()->data() + Offset + ChunkHeaderSize);
    ...
    const uint8_t* WAVDataProvider::GetWaveData() const {
        ...
        if (IsJUNK) {
            ...
            return reinterpret_cast<const uint8_t*>(
                m_data ? m_data.get()->data() + 44 /** standard wav header size */
                       : nullptr
            );
        }
    }

    size_t WAVDataProvider::GetWaveDataSize() const { return m_dataSize; }

    size_t WAVDataProvider::StreamWaveData(size_t size) { return 0; }
