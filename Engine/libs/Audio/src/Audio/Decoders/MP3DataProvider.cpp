...
namespace SR_AUDIO_NS {
    ...
    MP3DataProvider::MP3DataProvider(const RawSoundDataPtr& data)
        : m_data(data), m_format(), m_decodingBuffer(MINIMP3_MAX_SAMPLES_PER_FRAME * 16), m_bufferUsed(0),
          m_streamPos(0), m_initialStreamPos(0), m_isEndOfStream(false), m_decoderData(new DecoderData()) {
        ...
        /** dec.samples, dec.info.hz, dec.info.layer, dec.info.channels should
         * be filled */
        ...
        m_bufferUsed =
            mp3dec_ex_read(&m_decoderData->mp3d, (mp3d_sample_t*)m_decodingBuffer.data(), m_decoderData->mp3d.samples);
        if (m_bufferUsed != m_decoderData->mp3d.samples) /* normal eof or error condition */
        {
            ...
            SR_ERROR("MP3DataProvider::MP3DataProvider() : samples count is "
                     "different!");
            return;
        }
    ...
    const uint8_t* MP3DataProvider::GetWaveData() const { return m_decodingBuffer.data(); }

    size_t MP3DataProvider::GetWaveDataSize() const { return m_bufferUsed; }

    void MP3DataProvider::Seek(float_t seconds) {}

    size_t MP3DataProvider::StreamWaveData(size_t size) { return IWaveDataProvider::StreamWaveData(size); }
} // namespace SR_AUDIO_NS