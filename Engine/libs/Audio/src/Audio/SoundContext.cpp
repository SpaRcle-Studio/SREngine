...
namespace SR_AUDIO_NS {
    ...
    SoundDevice* SoundContext::GetDevice() const { return m_device; }
    ...
    void SoundContext::ApplyParams(SoundSource pSource, const PlayParams& params) {
        SR_TRACY_ZONE;
        /*ApplyParam(pSource, params.device, m_params.device,
        PlayParamType::Device); ApplyParam(pSource, params.coneInnerAngle,
        m_params.coneInnerAngle, PlayParamType::ConeInnerAngle);
        ApplyParam(pSource, params.async, m_params.async, PlayParamType::Async);
        ApplyParam(pSource, params.direction, m_params.direction,
        PlayParamType::Direction); ApplyParam(pSource, params.gain,
        m_params.gain, PlayParamType::Gain); ApplyParam(pSource, params.library,
        m_params.library, PlayParamType::Library); ApplyParam(pSource,
        params.loop, m_params.loop, PlayParamType::Loop); ApplyParam(pSource,
        params.maxGain, m_params.maxGain, PlayParamType::MaxGain);
        ApplyParam(pSource, params.minGain, m_params.minGain,
        PlayParamType::MinGain); ApplyParam(pSource, params.orientation,
        m_params.orientation, PlayParamType::Orientation); ApplyParam(pSource,
        params.pitch, m_params.pitch, PlayParamType::Pitch); ApplyParam(pSource,
        params.position, m_params.position, PlayParamType::Position);
        ApplyParam(pSource, params.relative, m_params.relative,
        PlayParamType::Relative); ApplyParam(pSource, params.uniqueId,
        m_params.uniqueId, PlayParamType::UniqueId); ApplyParam(pSource,
        params.velocity, m_params.velocity, PlayParamType::Velocity);*/

        ApplyParam(pSource, params.device, PlayParamType::Device);
        // ApplyParam(pSource, params.coneInnerAngle,
        // PlayParamType::ConeInnerAngle);
