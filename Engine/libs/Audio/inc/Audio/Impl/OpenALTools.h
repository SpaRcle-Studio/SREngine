...
namespace SR_AUDIO_NS {
    typedef struct ALfVec3 {
        ALfloat vec3[3];
    } ALfVec3;
    typedef struct ALfVec6 {
        ALfloat vec6[6];
    } ALfVec6;
    ...
    ALCdevice* device)
    {
        ...
        if(!alcCall(alcGetString, devices, device, nullptr,
    ALC_DEVICE_SPECIFIER)) return false;
    ...
    void CheckThreadId(const char* filename, const std::uint_fast32_t line) {
        ...
        if (currentThreadId != soundThreadId) {
            SRHalt(
                "OpenAL error: {}:{}\n\tOpenAL function called from different "
                "thread!\n\tCurrent thread id: {}\n\tSound thread id: {}",
                filename, line, currentThreadId.c_str(), soundThreadId.c_str()
            );
        }
#endif
    }

    // this is here thanks to
    ...
    bool check_alc_errors(const std::string& filename, const std::uint_fast32_t line, ALCdevice* device) {
        ...
        if (error != ALC_NO_ERROR) {
            ...
            case ALC_INVALID_VALUE:
                message += "ALC_INVALID_VALUE: an invalid value was passed to "
                           "an OpenAL function";
                break;
            case ALC_INVALID_DEVICE:
                message += "ALC_INVALID_DEVICE: a bad device was passed to an "
                           "OpenAL function";
                break;
            case ALC_INVALID_CONTEXT:
                message += "ALC_INVALID_CONTEXT: a bad context was passed to "
                           "an OpenAL function";
                break;
            case ALC_INVALID_ENUM:
                message += "ALC_INVALID_ENUM: an unknown enum value was passed "
                           "to an OpenAL function";
                break;
            case ALC_OUT_OF_MEMORY:
                message += "ALC_OUT_OF_MEMORY: an unknown enum value was "
    ...
    bool check_al_errors(const std::string& filename, const std::uint_fast32_t line) {
        ...
        if (error != AL_NO_ERROR) {
            ...
            case AL_INVALID_NAME:
                message += "AL_INVALID_NAME: a bad name (ID) was passed to an "
                           "OpenAL function";
                break;
            case AL_INVALID_ENUM:
                message += "AL_INVALID_ENUM: an invalid enum value was passed "
                           "to an OpenAL function";
                break;
            case AL_INVALID_VALUE:
                message += "AL_INVALID_VALUE: an invalid value was passed to "
                           "an OpenAL function";
                break;
            case AL_INVALID_OPERATION:
                message += "AL_INVALID_OPERATION: the requested operation is "
                           "not valid";
                break;
            case AL_OUT_OF_MEMORY:
                message += "AL_OUT_OF_MEMORY: the requested operation resulted "
                           "in OpenAL running out of memory";
                break;
            default:
