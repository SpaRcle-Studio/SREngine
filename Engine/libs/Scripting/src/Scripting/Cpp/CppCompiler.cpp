...
namespace SR_SCRIPTING_NS {
    ...
    void CppCompiler::SaveSettings() {
        ...
        if (!serializer.SaveToFile(settingsPath)) {
            SR_WARN(
                "CppCompiler::SaveSettings() : failed to save settings file: "
                "{}",
                settingsPath
            );
        }
    ...
    bool CppCompiler::Init() {
        ...
        if (!IsCompilerAvailable()) {
            if (SR_PLATFORM_NS::GetType() == SR_UTILS_NS::PlatformType::Windows) {
                ...
                if (!IsCompilerAvailable()) {
                    SR_ERROR(
                        "CppCompiler::Init() : g++ compiler not found by path "
                        "{}! Please install g++ by command: sudo apt install "
                        "g++",
    ...
    bool CppCompiler::IsCompilerAvailable() const { return SR_PLATFORM_NS::IsExists(m_settings.compilerPath); }

    bool CppCompiler::InstallMinGW() {
        std::string url = "https://github.com/brechtsanders/winlibs_mingw/releases/download/"
        ...
        if (!SR_PLATFORM_NS::IsExists(zipFile)) {
            if (!SR_PLATFORM_NS::DownloadFile(url, zipFile)) {
                SR_ERROR(
                    "CppCompiler::InstallMinGW() : failed to download file "
                    "from url: " +
                    url
                );
                return false;
    ...
    std::string CppCompiler::GetCompilerVersion() const {
        ...
        if (version.empty()) {
            SR_ERROR("CppCompiler::GetCompilerVersion() : failed to get "
                     "compiler version!");
            return "";
        }
    ...
    bool CppCompiler::Compile(const CppCompilerContext& context) {
        ...
        if (m_settings.compilerType == CppCompilerType::MSVC) {
            ...
            customArgs += "-DFMT_HEADER_ONLY ";
            customArgs += "-std=c++20 ";
            customArgs += "-Wno-deprecated -Wno-unused-variable -Wno-parentheses "
                          "-Wno-deprecated-declarations -Wno-reorder "
                          "-Wno-unused-function -Wno-attributes -Wno-delete-incomplete "
                          "-Wno-sign-compare -Wno-unused-function "
                          "-Wno-overloaded-virtual -Wno-comment -Wno-template-body ";
        }
        ...
        if (m_settings.compilerType == CppCompilerType::MSVC) {
            ...
            std::string msvcInclude =
                m_settings.compilerPath.GetPrevious().GetPrevious().GetPrevious().GetPrevious().Concat("include");
            ...
            includePaths += "/I\"" +
                            SR_UTILS_NS::Path(SR_WINDOWS_SDK_DIR)
                                .Concat("Include")
                                .Concat(SR_WINDOWS_SDK_VERSION)
                                .Concat("ucrt")
                                .ToString() +
                            "\" ";

            windowsKitsUmLibs = SR_UTILS_NS::Path(SR_WINDOWS_SDK_DIR)
                                    .Concat("Lib")
                                    .Concat(SR_WINDOWS_SDK_VERSION)
                                    .Concat("um/x64")
                                    .ToString();
            windowsKitsUcrtLibs = SR_UTILS_NS::Path(SR_WINDOWS_SDK_DIR)
                                      .Concat("Lib")
                                      .Concat(SR_WINDOWS_SDK_VERSION)
                                      .Concat("ucrt/x64")
                                      .ToString();
#else
            SRHalt("CppCompiler::Compile() : windows latest SDK dir is not "
                   "defined!");
#endif

            std::string msvcLibs = m_settings.compilerPath.GetPrevious()
                                       .GetPrevious()
                                       .GetPrevious()
                                       .GetPrevious()
                                       .Concat("lib/x64")
        ...
        std::string command =
            "{} {} {} {} {}"_format(m_settings.compilerPath, customArgs, outArgs, sourceFiles, includePaths);

        if (m_settings.compilerType == CppCompilerType::MSVC) {
            // command += " /LIBPATH:\"C:/Program Files/Microsoft Visual
            // Studio/2022/Community/VC/Tools/MSVC/14.42.34433/lib/x64\"";
            if (context.isDebug) {
        ...
        const bool hasErrors = result.find("compilation terminated") != std::string::npos ||
        ...
        if (hasErrors) {
            SR_ERROR(
                "CppCompiler::Compile() : \"{}\" module compilation "
                "failed!\n{}",
    ...
    bool CppCompiler::FindEngineLibs() {
        ...
        for (auto&& libName : libs) {
            ...
            SR_ERROR(
                "CppCompiler::FindEngineLibs() : failed to find engine lib: {} "
                "or {}!",
                pathDebug, pathRelease
            );
            return false;
        }
    ...
    bool CppCompiler::FindWindowsCompiler() {
        if (!SR_PLATFORM_NS::IsCompiledUnderMSVC()) {
            SR_ERROR("CppCompiler::FindWindowsCompiler() : windows application "
                     "not compiled under MSVC!");
            return false;
        }
        ...
        if (SR_PLATFORM_NS::IsExists(builtInCompilerPath)) {
            const auto&& result = SR_PLATFORM_NS::ShowMessageBox(
                "Choose compiler",
                "MSVC built-in compiler is available. Do you want to use it? "
                "Use only if you are developer.",
                SR_PLATFORM_NS::MessageBoxType::YesNo, SR_PLATFORM_NS::MessageBoxIconType::Question,
        ...
        const auto&& result = SR_PLATFORM_NS::ShowMessageBox(
            ...
            "Do you want to select existing compiler? (Yes). Or you want to "
            "install it? (No)",
            SR_PLATFORM_NS::MessageBoxType::YesNoCancel, SR_PLATFORM_NS::MessageBoxIconType::Question,
            SR_PLATFORM_NS::MessageBoxDefaultButtonType::YesOk
        );

        if (result == SR_PLATFORM_NS::MessageBoxResultType::Cancel) {
            SR_ERROR("CppCompiler::FindWindowsCompiler() : user canceled "
                     "compiler selection!");
            return false;
        }
        ...
        if (result == SR_PLATFORM_NS::MessageBoxResultType::No) {
            SR_PLATFORM_NS::ShowMessageBox(
                ...
                "After press OK button, compiler installer will be launched, "
                "and current application will be closed. Please, restart "
                "application after installation.",
                SR_PLATFORM_NS::MessageBoxType::Ok, SR_PLATFORM_NS::MessageBoxIconType::Info,
                SR_PLATFORM_NS::MessageBoxDefaultButtonType::YesOk
            );
            auto&& pathToVSBuildToolsInstaller = m_resourcesPath.Concat("Engine/Utilities/vs_BuildTools.exe");
            SR_SYSTEM_LOG(
                "CppCompiler::FindWindowsCompiler() : path to VS Build Tools "
                "installer: " +
                pathToVSBuildToolsInstaller.ToStringRef()
            );
            SR_SYSTEM_LOG("CppCompiler::FindWindowsCompiler() : engine will be "
                          "terminated after installer launched!");
            SR_PLATFORM_NS::ExecuteCommand(pathToVSBuildToolsInstaller.ToStringRef());
            SR_PLATFORM_NS::Terminate(false);
        }

        if (result == SR_PLATFORM_NS::MessageBoxResultType::YesOk) {
            SR_LOG("CppCompiler::FindWindowsCompiler() : requesting compiler "
                   "path from user...");
            m_settings.compilerPath =
                SR_UTILS_NS::FileDialog::Instance().OpenDialog(m_resourcesPath, {{"cl.exe msvc compiler", "exe"}});
            if (!IsCompilerAvailable()) {
                SR_ERROR("CppCompiler::FindWindowsCompiler() : invalid "
                         "compiler path!");
                SR_PLATFORM_NS::ShowMessageBox(
                    "Invalid compiler", "Please select valid MSVC (cl.exe) compiler!",
                    SR_PLATFORM_NS::MessageBoxType::Ok, SR_PLATFORM_NS::MessageBoxIconType::Error,
                    SR_PLATFORM_NS::MessageBoxDefaultButtonType::YesOk
                );
                return false;
            }
    ...
    bool CppCompiler::ValidateCompilerAndOS() {
        if (m_settings.compilerType == CppCompilerType::MSVC) {
            if (SR_PLATFORM_NS::GetType() != SR_UTILS_NS::PlatformType::Windows) {
                SR_ERROR("CppCompiler::ValidateCompilerAndOS() : MSVC compiler "
                         "is not available on this platform!");
                return false;
            }

            if (!SR_PLATFORM_NS::IsCompiledUnderMSVC()) {
                SR_ERROR("CppCompiler::ValidateCompilerAndOS() : application is not "
                         "compiled under MSVC, but MSVC compiler is selected!");
                return false;
            }

            if (m_settings.compilerPath.GetBaseNameAndExt() != "cl.exe") {
                SR_ERROR(
                    "CppCompiler::ValidateCompilerAndOS() : invalid MSVC "
                    "compiler path! Path: {}",
                    m_settings.compilerPath
                );
                return false;
            ...
            const std::string buildInCompilerVersion =
                GetBuiltInMSVCCompilerPath().GetPrevious().GetPrevious().GetPrevious().GetPrevious().GetBaseNameAndExt(
                );
            const std::string currentCompilerVersion =
                m_settings.compilerPath.GetPrevious().GetPrevious().GetPrevious().GetPrevious().GetBaseNameAndExt();

            if (buildInCompilerVersion != currentCompilerVersion) {
                const std::string mismatchCompilerLogMessage =
                    "CppCompiler::ValidateCompilerAndOS() : built-in compiler "
                    "version is not equal to current compiler version! "
                ...
                if (!m_settings.ignoreCompilerVersion) {
                    ...
                    const std::string message =
                        "Do you want to continue? Continuing may cause undefined behaviour. Use on your own risk. Built-in version: {}, current version: {}"_format(
                            buildInCompilerVersion, currentCompilerVersion
                        );

                    const auto&& result = SR_PLATFORM_NS::ShowMessageBox(
                        "MSVC compiler version mismatch", message, SR_PLATFORM_NS::MessageBoxType::YesNo,
                        SR_PLATFORM_NS::MessageBoxIconType::Warning, SR_PLATFORM_NS::MessageBoxDefaultButtonType::No
                    );
                ...
                SR_INFO("CppCompiler::ValidateCompilerAndOS() : user accepted "
                        "to continue with different compiler version!");
            }
        } else if (m_settings.compilerType == CppCompilerType::GCC) {
            if (SR_PLATFORM_NS::GetType() != SR_UTILS_NS::PlatformType::Linux) {
                SR_ERROR("CppCompiler::ValidateCompilerAndOS() : GCC compiler "
                         "is not available on this platform!");
                return false;
            }
        } else {
            SR_ERROR(
                "CppCompiler::ValidateCompilerAndOS() : unsupported compiler "
                "type! Type: {}",
                m_settings.compilerType
            );
            return false;
        }
