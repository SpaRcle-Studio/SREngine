...
namespace SR_PTYPES_NS {
    ...
    bool PhysicsMaterial::Load() {
        ...
        SR_LOG(
            "PhysicsMaterial::Load() : the physics material is loading. "
            "\n\tPath: " +
            path.ToString()
        );
        ...
        if (!matXml) {
            SR_ERROR(
                "PhysicsMaterial::Load() : \"PhysicsMaterial\" node is not "
                "found! \n\tPath: " +
                path.ToString()
            );
            return false;
        ...
        SetFrictionCombine(SR_UTILS_NS::EnumReflector::FromString<Combine>(
            matXml.TryGetNode("FrictionCombine").TryGetAttribute<std::string>("Average")
        ));
        SetBounceCombine(SR_UTILS_NS::EnumReflector::FromString<Combine>(
    ...
    bool PhysicsMaterial::Save(const SR_UTILS_NS::Path& path, const PhysicsMaterialData& materialData) {
        ...
        matXml.AppendNode("FrictionCombine")
            .AppendAttribute(SR_UTILS_NS::EnumReflector::ToStringAtom(materialData.frictionCombine));
        matXml.AppendNode("BounceCombine")
            .AppendAttribute(SR_UTILS_NS::EnumReflector::ToStringAtom(materialData.bounceCombine));

        if (!document.Save(path)) {
            SR_ERROR(
                "PhysicsMaterial::Save() : failed to save the document! "
                "\n\tPath: " +
                path.ToString()
            );
            return false;
