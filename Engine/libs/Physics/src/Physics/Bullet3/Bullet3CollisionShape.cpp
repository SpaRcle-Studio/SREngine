...
namespace SR_PTYPES_NS {
    ...
    bool Bullet3CollisionShape::UpdateShape() {
        if (!m_library->IsShapeSupported(m_type)) {
            SR_WARN("Bullet3CollisionShape::Update() : shape is not supported! "
                    "Replace to default...");
        ...
        default:
            SR_ERROR(
                "Bullet3CollisionShape::Update() : unsupported shape! Type: " +
                SR_UTILS_NS::EnumReflector::ToString(m_type)
            );
            return false;
        }
